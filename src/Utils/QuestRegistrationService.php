...
class QuestRegistrationService
{
    ...
    public function inscriptionVerif(Quest $quest, User $user){
        ...
        if ($quest->getNbMaxInscription() <= $quest->getUsers()->count()
            || $quest->getUsers()->contains($user)
            || $quest->getInscriptionLimitDate() < new \DateTime()
            || $quest->getStatus()->getLabel() === 'Annulée')
        {
            return false;
        } else {
    ...
    public function desisterVerif(Quest $quest, User $user){
        ...
        if ($quest->getUsers()->contains($user)
            && $quest->getStartDateTime() > new \DateTime()
            && $quest->getStatus()->getLabel() != 'Annulée')
        {
